...
class Store {
  ...
  dispatch(action) {
    this.state = this.reducer(this.state, action); // this._notifyListeners();
  }
...
class View {
  constructor(el, store) {
    this.el = el;
    this.store = store; // this.unsubscribe = store.subscribe(this.prepareRender.bind(this) );
    // this.prepareRender(this.store.getState());
  } // prepareRender(state) {
  //     this.render();
  // }
...
searchInput.addEventListener('input', function () {
  ...
  view.render(); // view.prepareRender(store.getState())
});